...
class SubsequentBlock extends Component {
    ...
    state = {
        ledgerPayload: '',
        token: '',
        sha256Output: sha256(this.props.previousHash),
        concatenatedString: this.props.previousHash,
        showMineButton: true
    };
    ...
    handleClickAndStartMining = () => {
        ...
        if (this.state.showMineButton === true) {
            ...
            document.getElementById("subsequentPayload").readOnly=true;
            document.getElementById("subsequentToken").readOnly = true;
            ...
            this.setState({ token: '', showMineButton: false }, () => {
                ...
                console.log("From SubsequentBlock handleClick(): ", hash.substring(0,2));

                while (hash.substring(0,4) !== "0000") {
                    token = i.toString();
                    concatenatedString = payload + token + previousHash;
    ...
    concatenateStringsForSHA256Input = () => {
        console.log("From inside SubsequentBlock concatenateStringsForSHA256Input: I ran")
        ...
        if (this.state.ledgerPayload === '' && this.state.token === '') {
            this.setState({ sha256Output: sha256(this.props.previousHash)}); 
        } else {
            let concatenatedString = this.state.ledgerPayload + this.state.token + this.props.previousHash;
            this.setState({
        ...
        return (
                <div style={styles.formContainer}>
                    <form>
                        ...
                        <p>
                            <button id="subsequentBlockButton" type="button" style={styles.button} onClick={this.handleClickAndStartMining}> 
                                Start Mining!
                            </button>
                        </p>
...
function mapStateToProps (state) {
  ...
  export default connect(mapStateToProps, mapDispatchToProps)(SubsequentBlock)